...
def files():
    ...
    default_directory = '/home/user/kismet'
    
    files = get_files_from_directory(current_directory or default_directory)
    ...
    try:
        ...
        with zipfile.ZipFile(archive_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for filename in os.listdir(log_dir):
                if filename != archive_name:  # Don't include the archive itself
...
def convert_to_kml():
    ...
    try:
        ...
        # Handle empty directory (current directory)
        if not directory:
            directory = '.'
        ...
        if not os.path.exists(input_path):
            # Also check in common Kismet directories
            common_dirs = [
            ...
            for common_dir in common_dirs:
                ...
                if os.path.exists(test_path):
                    ...
                    output_path = os.path.join(common_dir, output_filename)
                    found = True
                    app.logger.info(f"Found file in {common_dir}")
                    break
