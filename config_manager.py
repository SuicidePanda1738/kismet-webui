...
class ConfigManager:
    ...
    def __init__(self):
        ...
        # Cache the parsed config briefly so repeated reads within a short
        # window skip re-reading and re-parsing the config files. Invalidated
        # whenever save_config() writes a new configuration.
        self._config_cache = None
        self._config_cache_time = 0.0
        self._CONFIG_TTL = 30  # seconds

    def load_config(self) -> Dict[str, Any]:
        """Load current Kismet configuration"""
        now = time.monotonic()
        if self._config_cache is not None and (now - self._config_cache_time) < self._CONFIG_TTL:
        ...
        try:
            ...
            for config_path in self.config_paths:
                try:
                    ...
                    # New config on disk; drop the cached parse so the next
                    # load_config() reflects it immediately.
                    self._config_cache = None
    ...
    def _generate_config_content(self, config_data: Dict[str, Any]) -> str:
        ...
        if gps_type in ('gpsd', 'remote'):
            ...
            lines.append(f'gps=gpsd:host={gps_host},port={gps_port},reconnect=true')
